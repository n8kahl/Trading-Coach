...
- `FF_CHART_CANONICAL_V1` – emit canonical `/chart` links and persist `plan_layers` with every plan.
...
- Without a live database the service falls back to in-memory caching, and `/chart` overlays break after restarts.
...
- Charts: canonical `/chart` URLs only; no `session_*` or inline levels in links. Overlays fetched by `plan_id` via `GET /api/v1/gpt/chart-layers`.
...
└── src/
    ├── agent_server.py      # FastAPI app (all /gpt routes + tv-api datafeed)
...
BASE_URL=https://<your-app>.up.railway.app/chart
...
PUBLIC_BASE_URL=https://trading-coach-production.up.railway.app    # Absolute base used for link unfurls (/chart)
...
`/chart` is the canonical interactive viewer for plans and scans. Legacy Idea Page bundles have been removed; continue using the `/gpt/chart-url` surface to build shareable `/chart` links.
...
10. Copy the canonical chart link and open it in a new tab; `/chart?...` loads bars within ~2 s, overlays arrive after the initial batch, and the embed stays on the live resolution.
```
...
- **FastAPI app (`src/agent_server.py`)** – hosts GPT endpoints and the `/tv-api` datafeed. Uses async helpers for I/O.
...
- **Chart renderer (`static/tv`)** – Legacy Lightweight Charts assets retained for reference; production embeds use the Next.js `/chart` page powered by the `/tv-api` datafeed.
...
- Plans include a banner (`fallback_banner`) and settlement note to flag American-style ETF contracts; charts receive the same metadata for the `/chart` renderer.
...
- Generate shareable charts using /gpt/chart-url and present the returned /chart link (plan overlays fetched by `plan_id`).
   ```

...
def test_make_chart_url_orders_params_and_formats_precision():
    ...
    url = make_chart_url(params, base_url="https://example.com/webview/index.html", precision_map={"SPY": 2})

    assert (
        url
        == "https://example.com/webview/index.html?center_time=latest&direction=long&ema=9,21&entry=430.12&focus=plan&interval=5m&plan_id=SPY-PLAN&plan_version=2&scale_plan=auto&stop=428.99&symbol=SPY&theme=dark&tp=432.55,435"
    )
...
def test_make_chart_url_ignores_non_allowlisted_keys():
    url = make_chart_url(
        ...
        base_url="https://example.com/webview/index.html",
        precision_map={"NDX": 2},
    )
...
def test_make_chart_url_includes_ui_state_token():
    ...
    url = make_chart_url(params, base_url="https://example.com/webview/index.html", precision_map={"AAPL": 2})

...
def test_chart_tokenization_includes_h1_h4_and_pivots() -> None:
    ...
    assert any("|H1Pivot" in token for token in tokens)
    assert any("|H1R1" in token for token in tokens)
    assert any("|H1S1" in token for token in tokens)
    assert any("|H4Pivot" in token for token in tokens)

 ...
 * Only allows server-hosted paths that start with /chart.
 * Returns a safe fallback '/chart' on invalid input.
 */
export function validateChartUrl(url: string): string {
  try {
    const u = new URL(
      url,
      typeof window !== 'undefined' ? window.location.origin : 'http://localhost'
    );
    if (!u.pathname.startsWith('/chart')) throw new Error('Non-canonical chart URL');
